...
if ($result === FALSE) {
    echo "cURL Error: 33 " . curl_error($curl); die();
}
...
if ((string)$xmlObject->ErrorCode !== 'Ok') {
   ...
   echo "Error: 44 " . $xmlObject->ErrorDescription; die();
}
...
$result = curl_exec($curl);
if ($result === FALSE) {
    echo "cURL Error: 118" . curl_error($curl); die();
}
$xmlObject = simplexml_load_string($result);
...
$result = curl_exec($curl);
if ($result === FALSE) {
    echo "cURL Error: 663 " . curl_error($curl); die();
}
$xmlObject = simplexml_load_string($result);
...
if ((string)$xmlObject->ErrorCode !== 'Ok') {
    echo "Error: 673 " . $xmlObject->ErrorDescription; die();
}
    ...
    if(imagelink==null ){ imagelink = "<?php echo $itemInfo['MainPictureUrl'];?>";}  
      ...
      var str = "price=" + toatalPrice + "&quantity=" + productQuantity + "&title=" + producttitle2 + "&link=" + productlink + "&content=" + txt + "&image=" + imagelink + "&consoins=" + conso_ins+ "&physical="+ phy_param;  
       ...
       var xmlhttp = new XMLHttpRequest();
        ...
        //xmlhttp.open("GET", "https://borderlesscommerce.net/saveproduct.php?"+str, true);   ///wp-content/themes/porto-child/
...
function GetFileUploadUrl($filename)
{
    ...
    if ($result === FALSE) {
    echo "cURL Error: 1164" . curl_error($curl); die();
                       }
    $xmlObject = simplexml_load_string($result);
    ...
    if ((string)$xmlObject->ErrorCode !== 'Ok') 
    {
   echo "Error: 1175" . $xmlObject->ErrorDescription; die();
     }
...
function GetFileInfo($fileid)
{
   //http://otapi.net/OtapiWebService2.asmx/GetFileInfo?language=ru&fileId=e6a05dcd-d345-eb11-80c4-f409ed584015&instanceKey=opendemo
    ...
    $result = curl_exec($curl);
    if ($result === FALSE) {
    echo "cURL Error: 1219" . curl_error($curl); die();
                       }
    $xmlObject = simplexml_load_string($result);
    ...
    if ((string)$xmlObject->ErrorCode !== 'Ok') 
    {
   echo "Error: 1231 " . $xmlObject->ErrorDescription; die();
     }
...
if (isset($_FILES['fileToUpload']['name']) && $_FILES['fileToUpload']['name']!="") 
{
    ...
    // New Fitler Code 
       $brand_id = empty($_GET['BrandId'])?'':$_GET['BrandId'];
       $cat_id = empty($_GET['CatId'])?'':$_GET['CatId'];
       ...
       $brand_id_api_var = empty($brand_id)?'' : '<BrandId>'.$brand_id.'</BrandId>';
       $cat_id_api_var = empty($cat_id)?'' : '<CategoryId>'.$cat_id.'</CategoryId>';
    ...
    // New Fitler Code 
// $_SESSION['imgurl'] = 'https://www.filmibeat.com/ph-big/2020/02/v-2020_158253142110.jpg';   // Temporory for imagage search only
   if(isset($_SESSION['imgurl']) && $url==""){
    
    $xmlp='<SearchItemsParameters><Provider>Taobao</Provider>'.$brand_id_api_var.$cat_id_api_var.$price_range_min_api_var.$price_range_max_api_var.$price_order_api_var.'<ImageUrl>'.$_SESSION['imgurl'].'</ImageUrl></SearchItemsParameters>';
...
if ($result === FALSE) {
    echo "cURL Error: 1298" . curl_error($curl);
}
$xmlObject = simplexml_load_string($result);
...
if ((string)$xmlObject->ErrorCode !== 'Ok') {
    echo "Errors: " . $xmlObject->ErrorDescription;
                ...
                // get brand for filter # New Api call code for Image Search
            
    /******************************** brand code *****************************  */
    
            $xmlObject_brand = return_api_data('GetBrandInfoList');        
    
            $json_brand = json_encode($xmlObject_brand);
            $data_brand = json_decode($json_brand,true);
    
            if ((string)$xmlObject_brand->ErrorCode !== 'Ok') {
                echo "Error: 1319 " . $xmlObject_brand->ErrorDescription; print_r($xmlObject_brand); echo "<br> last else "; die();
            }
            
            $brand_list = $data_brand['BrandInfoList']['Content']['Item'];
    
    /******************************** Category code *****************************  */
    
            $xmlObject_category = return_api_data('GetRootCategoryInfoList');
    
            // echo "<pre>"; print_r($xmlObject_category); echo "</pre>";
    
            $json_category = json_encode($xmlObject_category);
            $data_category = json_decode($json_category,true);
             
             
            if ((string)$xmlObject_category->ErrorCode !== 'Ok') {
                echo "Error: 1335" . $xmlObject_category->ErrorDescription; print_r($xmlObject_category); echo "<br> last else "; die();
            }
            
            $category_list = $data_category['CategoryInfoList']['Content']['Item'];
    
            // New brand api call end for Image Search              
...
// $xmlp='<SearchItemsParameters><Provider>Taobao</Provider><SearchMethod>Extended</SearchMethod><IsClearItemTitles>false</IsClearItemTitles><ItemTitle>'.$url1.'</ItemTitle></SearchItemsParameters>';  // comment by rsw
$xmlp='<SearchItemsParameters><Provider>Taobao</Provider><SearchMethod>Extended</SearchMethod><IsClearItemTitles>false</IsClearItemTitles><ItemTitle>'.$url1.'</ItemTitle>'.$brand_id_api_var.''.$cat_id_api_var.''.$price_order_api_var.''.$price_range_min_api_var.''.$price_range_max_api_var.'</SearchItemsParameters>';
$xmlpl = str_replace("<","%3c",$xmlp);
$xmlpr = str_replace(">","%3e",$xmlpl);
...
$result = curl_exec($curl);
if ($result === FALSE) {
    echo "cURL Error: 1370" . curl_error($curl); die();
}
$xmlObject = simplexml_load_string($result);
...
if ((string)$xmlObject->ErrorCode !== 'Ok') {
    echo "Error: 1378" . $xmlObject->ErrorDescription; die();
}
     
                // get brand for filter # New Api call code
...
/******************************** brand code *****************************  */
        ...
        if ((string)$xmlObject_brand->ErrorCode !== 'Ok') {
            echo "Error: 1391" . $xmlObject_brand->ErrorDescription; print_r($xmlObject_brand); echo "<br> last else brand "; die();
        }
...
/******************************** Category code *****************************  */
        ...
        if ((string)$xmlObject_category->ErrorCode !== 'Ok') {
            echo "Error: 1405" . $xmlObject_category->ErrorDescription; print_r($xmlObject_category); echo "<br> last else category "; die();
        }
        
        $category_list = $data_category['CategoryInfoList']['Content']['Item'];

        // New brand api call end
...
<div>
    ...
    <script src="https://cdn.jsdelivr.net/npm/select2@4.0.13/dist/js/select2.min.js"></script>
     ...
     <h3>Filters</h3>
            <form action="https://borderlesscommerce.net/taobao-to-borderless-commerce/" method="get">
...
/***************** new url logic *********************/
       ...
       $brand_id_URL = empty($brand_id)?'': '&BrandId='.$brand_id;
       $cat_id_URL = empty($cat_id)?'': '&CatId='.$cat_id;
    ...
    $pagination_parameters = $url_parameter.$brand_id_URL.$cat_id_URL.$price_order_URL.$price_min_URL.$price_max_URL;
...
// 			$paginationURL_p = 'https://borderlesscommerce.net/taobao-to-borderless-commerce/?from='.$from_p.'&url='.$url;
			...
			$paginationURL = 'https://borderlesscommerce.net/taobao-to-borderless-commerce/?from='.$from.$pagination_parameters;
			$paginationURL_n = 'https://borderlesscommerce.net/taobao-to-borderless-commerce/?from='.$from.$pagination_parameters;
			$paginationURL_p = 'https://borderlesscommerce.net/taobao-to-borderless-commerce/?from='.$from.$pagination_parameters;
	 ?>
    ...
    <div class="<?php echo $show_hide; ?>">
		<div class="pagination">
		  ...
		  <a href="<?php echo $paginationURL_p;?>">&laquo;</a>
			...
			echo	'<a href="https://borderlesscommerce.net/taobao-to-borderless-commerce/?from='.$from1.''.$pagination_parameters.'" class="'.$class.'">'.$i.'</a>';
...
function return_api_data($api_key)
{
    $link = 'http://otapi.net/OtapiWebService2.asmx/'.$api_key.'?instanceKey='.CFG_SERVICE_INSTANCEKEY.'&language='.CFG_REQUEST_LANGUAGE;
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $link);
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($curl, CURLOPT_HEADER, 0);
    ...
    $result = curl_exec($curl);
    if ($result === FALSE) {
        echo "cURL Error: 1658" . curl_error($curl); die();
    }
    curl_close($curl);
